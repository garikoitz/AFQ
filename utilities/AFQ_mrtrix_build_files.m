...
%     files.csfResponse = strcat(fname_trunk, '_csfResponse.txt');
    ...
    % Ahora para la tesis lo vamos a hardcodear
    disp(['OJO HARDCODEADO PARA MINI: PATH A LOS RESPONSES']);
    camino = '/bcbl/home/home_g-m/glerma/00local/PROYECTOS/MINI/ANALYSIS/DWI/output_group_average_response';
    files.wmResponse = [camino filesep 'wm_output_group_average_response.txt'];
    files.gmResponse = [camino filesep 'gm_output_group_average_response.txt'];
    files.csfResponse = [camino filesep 'cs_output_group_average_response.txt'];
